...
rl.greatestCommandDisplayNameLength = 0
...
rl.addCommands = (function () {
  var commandSchema = {
    ...
    argNames: { type: Array, arrayType: String, optional: true },
...
rl._printUsage = function () {
  ...
  for (var c = 0, commandsLen = this.commandNames.length; c < commandsLen; ++c) {
    ...
    var commandDisplayNames = this.commandDisplayNames[commandName]

    // Align command descriptions.
    var padding = Array(this.greatestCommandDisplayNameLength - commandDisplayNames.length + 1).join(' ') + '  '

    util.log('  ' + commandDisplayNames + padding + this.commandDescriptions[commandName])
  }
