...
rl.addCommands = function () {
  ...
  for (var c = 0, commandsLen = commands.length; c < commandsLen; ++c) {
    ...
    this.commands[command.name] = command.func

    // Save command name for `rl.completer` `<tab>` autocompletion.
    this.commandNames.push('.' + command.name)

    // Save command description for RLI usage screen.
    this.commandDescriptions[command.name] = command.description
  }
}
 ...
 * @static
 * @memberOf rl
 * @param {Function} func The function invoked per RLI input that is not a registered command. Passed the input line as the only argument.
 * @example
 *
